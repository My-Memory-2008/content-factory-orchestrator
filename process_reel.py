...
def commit_changes(reel_link, video_path=None):
    """
    Safely handles Git configuration, checks for variations, pulls remote 
    updates using a rebase approach, and pushes changes using token credentials.
    """
    try:
        token = os.environ.get("GITHUB_TOKEN")
        repo_slug = os.environ.get("GITHUB_REPOSITORY")
        active_branch = os.environ.get("GITHUB_REF_NAME", "main")

        # 1. Configure the official bot identifier metrics
        subprocess.run(["git", "config", "--global", "user.name", "github-actions[bot]"], check=True)
        subprocess.run(["git", "config", "--global", "user.email", "41898282+github-actions[bot]@://github.com"], check=True)
        
        # 2. Add files to the staging index layout
        subprocess.run(["git", "add", "input_link.txt", "reel_source_summa.txt", "rejected.txt"], check=True)
        if video_path and os.path.exists(video_path):
            subprocess.run(["git", "add", video_path], check=True)
            
        # 3. Verify if modifications exist before generating a commit snapshot
        status = subprocess.run(["git", "diff", "--cached", "--quiet"])
        if status.returncode == 1:
            subprocess.run(["git", "commit", "-m", f"Automated Pipeline: Processed single reel {reel_link}"], check=True)
            
            # 4. Grab latest remote modifications to avoid out-of-sync branch rejections
            subprocess.run(["git", "pull", "--rebase", "origin", active_branch], check=True)
            
            # 5. Push safely via authenticated string if running inside a remote GitHub Runner wrapper environment
            if token and repo_slug:
                auth_url = f"https://x-access-token:{token}@://github.com{repo_slug}.git"
                subprocess.run(["git", "push", auth_url, f"HEAD:{active_branch}"], check=True)
            else:
        ...
        print(f"❌ Git tracking flow terminated with an error code: {e}")

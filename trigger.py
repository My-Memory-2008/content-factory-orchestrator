...
def trigger_via_api():
    ...
    # 3. Read the pipeline metadata file generated by your workflow steps
    try:
        with open("pipeline_data.json", "r") as f:
            pipeline_data = json.load(f)
        print("📋 Loaded metadata configuration from pipeline_data.json.")
    except Exception as e:
        print(f"⚠️ Could not load pipeline_data.json ({e}). Falling back to manual trigger template.")
        pipeline_data = {}

    # 4. Build the exact Kernel Push Payload (Mirrors pressing 'Save Version')
    # This automatically updates your notebook workspace code dynamically before execution
    payload = {
        ...
        "language": "python",
        "isPrivate": True,
        "enableGpu": True,       # Locks in your target T4 runtime acceleration
        "enableInternet": True,
        "datasetDataSources": ["muhammadasjad2008/cat-reactions-vault"],
        "competitionDataSources": [],
        "kernelDataSources": [],
        # Injects runtime tracking parameters as environment data text safely
    ...
    # 5. Execute secure HTTP Basic Request directly into Kaggle's engine backend
    ...
    if response.status_code == 200:
        ...
        print(f"Kaggle is now processing version {result.get('versionNumber', 'New')} on your T4 GPU allocation.")

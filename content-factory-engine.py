...
# ==============================================================================
# PART 1 OF 2: MULTI-FRAME TIMELINE INTERNET AGENT DISCOVERY & ANGLE ANALYSIS
# ==============================================================================
...
if 'output_path' not in locals() and 'output_path' in globals():
...
web_contexts = {}
# NEW: Fallback tracking range parameters
detected_angle_range = [0, 45]

try:
    MODEL_ID = "google/gemma-4-E4B-it"
    print("🔄 Initializing Gemma 4 Multimodal Architecture for Full Scan...")
    processor = AutoProcessor.from_pretrained(MODEL_ID)
    model = AutoModelForZeroShotObjectDetection.from_pretrained(MODEL_ID) if 'AutoModelForZeroShotObjectDetection' in locals() else AutoModelForMultimodalLM.from_pretrained(MODEL_ID, dtype=torch.bfloat16, device_map="auto")
    ...
    for idx, pct in enumerate(SCAN_INTERVALS):
        ...
        # [UPDATED PROMPT]: Forces Gemma to analyze layout tilt and output an orientation search window
        discovery_prompt = (
            "Examine this frame. Identify if there is a creator username handle, watermark, or logo text.\n"
            "Estimate its rotation angle orientation from 0 to 360 degrees. Provide a strict 45-degree search range covering it (e.g., [45, 90], [180, 225]).\n"
            "Return the data strictly inside a raw JSON map schema:\n"
            "{\"found\": true, \"watermark_text\": \"@name\", \"requires_web_search\": true, \"angle_min\": 45, \"angle_max\": 90}"
        )
        ...
        with torch.inference_mode():
            gen_ids = model.generate(**inputs, max_new_tokens=150, do_sample=False, use_cache=True)
        ...
        if json_match:
            try:
                ...
                if data.get("found") and data.get("watermark_text"):
                    ...
                    # Pull dynamic angle boundaries directly from Gemma's geometric prediction
                    if "angle_min" in data and "angle_max" in data:
                        detected_angle_range = [int(data["angle_min"]), int(data["angle_max"])]
...
pipeline_state = {
    ...
    "watermark_angle_range": detected_angle_range, # Exporting angle boundaries to downstream pipeline cells
...
print(f"💾 Deep Scan Complete. Total unique watermarks discovered: {len(discovered_handles)} | Angular Target Window: {detected_angle_range}")
...
# Extract dynamic angular boundaries passed from Phase 1
watermark_angle_range = state.get("watermark_angle_range", [0, 45])
...
try:
    ...
    for idx in range(frames_to_process):
        ...
        vision_prompt = (
            "Analyze this video frame layout. Your goal is to track down the exact borders enclosing the text watermark handles like the words that start with @ , _, or any special symbol.Don't find the coordinations of the subtitles or captains that are in the video.\n"
...
#         "oddly satisfying asmr trending search terms keywords"
#     ]
#     collected_trends = []
...
#         seo_prompt = (
#             f"You are a real, independent creator running a viral Shorts channel with millions of views. "
...
#             f"- Use casual slang, raw observations, and conversational text (e.g., 'literally', 'physically feel', 'lowkey').\n"
...
#             f"{{\n"
#             f"  \"youtube_title\": \"Write an unpolished, incredibly punchy title under 55 characters that triggers an intense urge to watch. Use high-engagement structures like: 'This video literally resets your brain chemistry 🤯 #shorts', 'Why does this loop feel so illegal to watch? #shorts', or 'I can physically feel this video right now #shorts'. Choose the best stylistic match for the frame.\",\n"
#             f"  \"youtube_description\": \"Write a casual, 6-sentence human description designed to index for search traffic while looking 100% written by a real person. Sentence 1: A relatable observation or thought about the sensory feeling of this specific video clip. Sentence 2: Naturally mention high-volume search phrases that people actually type in when they are bored or cannot sleep (e.g., 'oddly satisfying kinetic sand cutting video', 'relaxing sand layering asmr compilation', 'satisfying slime noises'). Do not list them, write them as a natural casual sentence. Sentence 3: Include the exact mandatory credit link: 'Original concept inspired by @{username}'. Sentence 4: Append 5 high-traffic hashtags like #shorts #asmr #satisfying #oddlysatisfying #relaxing.\",\n"
#             f"  \"youtube_tags\": [\"Provide exactly 17 clean, flat keyword strings. Do not stack them. Blend general tags with specific long-tail human search queries like 'videos to fall asleep to', 'satisfying clips for when you are bored', 'relaxing sounds for anxiety', 'kinetic sand satisfying slicing'.\"]\n"
#             f"}}\n\n"
...
# TARGET_UPLOAD_FILE = "/kaggle/working/edited_source_only.mp4"

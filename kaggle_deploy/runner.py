#!/usr/bin/env python3
# runner.py - Complete pipeline for Kaggle execution
# This is the code that runs on Kaggle when triggered

import os, json, re, requests, subprocess, time, random, torch, sys, hashlib
...
print("🚀 Content Factory Engine started...")
...
LEDGER_PATH = os.path.join(WORKING_DIR, '.pipeline_hash.json')
...
# ==========================================
# 2. SMART CHECK - Exit if no new data
# ==========================================
...
try:
    ...
    print(f"⚠️ Could not fetch pipeline: {e}. Proceeding anyway.")
    pipeline = {}
    current_hash = None

# Check hash ledger
if current_hash and os.path.exists(LEDGER_PATH):
    try:
        with open(LEDGER_PATH, 'r') as f:
            last_hash = json.load(f).get('hash')
        if last_hash == current_hash:
            print(f"✅ No new data (hash unchanged). Exiting in 5s.")
            time.sleep(5)
            sys.exit(0)
    except:
        pass
...
if current_hash:
    ...
    print(f"🎯 New data detected! Proceeding...")
...
# ==========================================
# 5. FFMPEG RENDER (9:16 + Captions + Branding)
# ==========================================
